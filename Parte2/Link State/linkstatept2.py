...
class LinkStateRouting(slixmpp.ClientXMPP):
    ...
    def start(self, event):
        self.send_presence()
        self.get_roster()
        self.simulate()  # Generamos todas las tablas de enrutamiento

    def message_handler(self, msg):
        if msg['type'] in ('chat', 'normal'):
            from_node_jid = str(msg['from']).split('/')[0]
            to_node = [k for k,v in self.names.items() if v == from_node_jid][0]

            if to_node in self.all_routing_tables:
                route = self.all_routing_tables[to_node].get(from_node)
                if route:
                    print(f"Enviando mensaje de {from_node} a {to_node} a través de la ruta: {route}")
                    self.send_message(mto=self.names[to_node], mbody=msg['body'])
                else:
